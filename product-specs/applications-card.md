...
| Started, not submitted — draft without measurable steps | Not submitted               | "Application started Sep 18"                        |
| Submitted                                               | Applied                     | "Applied just now"                                  |
| Under review                                            | In review                   | "Applied 2 days ago"                                |
...
| Interview completed, review owner unconfirmed           | In review                   | "Interview completed"                               |
| Interview completed, partner owns next review           | In review                   | "Interview completed · Awaiting partner review"     |
| Interview completed, Verita owns next review            | In review                   | "Interview completed · Awaiting Verita review"      |
| On hold                                                 | On hold                     | "Placed on hold Sep 18"                             |
| Offer received                                          | Offer received              | "Offer received Sep 18"                             |
| Not selected                                            | Not selected                | "Application closed Sep 18"                         |
| Withdrawn                                               | Withdrawn                   | "Withdrawn Sep 18"                                  |

> Post-interview evaluation reuses `In review`, not a new `Interview completed` status: once an interview is completed and the application moves back into evaluation, `application-status` reverts to **`In review`** (`SCORING_PENDING`/`UNDER_REVIEW`, already in the [`engagements.md` §8](engagements.md#8-application-status-enum) enum) rather than introducing a distinct `Interview completed` status. This avoids adding an unnecessary status the enum doesn't need — the underlying system state genuinely is "under review" again, just with different prior context. `supporting-text` carries that context instead:
>
> - **Review owner confirmed as the partner:** "Interview completed · Awaiting partner review."
> - **Review owner confirmed as Verita:** "Interview completed · Awaiting Verita review."
> - **Review owner not confirmed/available:** "Interview completed" alone — the card must not guess or default to naming an owner it can't confirm.
>
...
> - **Draft without measurable steps:** `supporting-text` falls back to the last confirmed event — when the application was started, e.g. "Application started Sep 18" — never an invented or zero-value step count standing in for data that doesn't exist.
>
...
> `On hold`'s `supporting-text` stays event-only until a reason/owner is confirmable: unlike the post-interview `In review` rows (§2.4.1), no reason or owner (partner vs. Verita) is currently captured for why an application goes `ON_HOLD` — it's presently just a flag, per [`engagements.md` §8](engagements.md#8-application-status-enum)'s "parked for later" definition. `supporting-text` therefore stays at the minimal, honest fallback — "Placed on hold [date]" — naming only the confirmed event and date, never an invented reason or actor. If the underlying data model later captures a specific reason (e.g. "role requirements changed") or owner (e.g. "paused by [Partner]"), `supporting-text` should upgrade to name it, following the same confirmed-data-only pattern already established for the post-interview `In review` rows above — but that data does not exist today, so the richer copy is not yet implementable.
...
**Event-specific, not a generic timestamp:** where `supporting-text` reports the last meaningful event (the "Under review" and "Not selected" rows above), it names the actual event — never a vague "Application updated X ago," which tells the professional something changed without saying what or who initiated it. For example:

| Event                                | Supporting text (example)         |
| ------------------------------------ | --------------------------------- |
| Professional submitted application   | "Applied 2 days ago"              |
| Verita started reviewing             | "Review started 2 days ago"       |
| Partner requested an interview       | "Interview requested 2 days ago"  |
| Professional completed an assessment | "Assessment submitted 2 days ago" |
| No meaningful event since applying   | "Applied 2 days ago"              |

Name the actor (Verita, the partner, or the professional) only when it adds useful context and the system can confirm it — the card must not invent an actor or imply activity that didn't happen. When no event more recent than the original submission is confirmed, `supporting-text` falls back to the submission event ("Applied 2 days ago") rather than a generic "updated" phrasing, since that's the last event the card can actually stand behind.
...
For the "Under review, additional requirements" scenario (§2.4.1), where the card needs to communicate both the requested step and the professional's progress against it, that's still just one `supporting-text` line — its _content_ combines both pieces into a single string (e.g. "Complete your assessment · 2 of 4 steps completed"), the same way other rows in §2.4.1's matrix already combine an event and a detail into one line (e.g. "Interview completed · Awaiting partner review"). This is a content/copy concern, not a second component element.
...
**Supporting-text must reflect the actual event, independent of that decision:** regardless of which system status triggers the status-label transition, `supporting-text` must never claim review has started unless a review-start event is actually confirmed:

- **Review-start event confirmed:** "Review started 2 days ago."
- **No review-start event confirmed:** falls back to the submission event, "Applied 2 days ago" — per §2.4's existing event-specific rule, never a vague "In review" restatement standing in for an unconfirmed event.

This is the same fallback the §2.4.1 matrix's plain "Under review" row already uses ("Applied 2 days ago") — that example was already correct under this rule; this section makes the rule itself explicit rather than leaving it implicit in one example.
...
| **Update**            | Last meaningful update, timestamp                                                       | Show the most recent substantive change, not a generic "last viewed" timestamp.                                                                                                                                               |
