...
- A task-delivery deadline is only surfaced once delivery is the actually relevant next step for that stage — not while the application is still, say, `Applied` and delivery isn't yet the professional's concern.
...
| Interview completed, partner owns next review           | In review                   | "Awaiting partner review after your Sep 18 interview" |
| Interview completed, Verita owns next review            | In review                   | "Awaiting Verita review after your Sep 18 interview" |
...
> - **Review owner confirmed as Verita:** "Awaiting Verita review after your {date} interview."
...
> ✅ **Resolved (2026-09-23) — the badge names the interview step:** the interview stage uses two labels, so the badge itself says where the interview stands: **`Interview · Action required`** while the professional still has to book it, and **`Interview scheduled`** once a time is confirmed. After the interview, the status becomes **`In review`** (resolved note above). Because the badge already says "scheduled", the scheduled row's `supporting-text` carries only the date and time (e.g. "Sep 24 at 10 AM EDT"), not "Interview scheduled for…". This supersedes the earlier rule that `Interview` was the single label across every interview condition. Never use a variant like "In interview".

**`Action required` supporting-text examples:** the `supporting-text` line for a standalone `Action required` status names the one thing the professional owes before any interview. Approved examples:
...
- "Complete your background check", only when the professional has something to do. A check run by a third party with nothing for the professional to do is not `Action required`.

Copy rules for this line:

- **Name the action, not the event.** "Complete your assessment", not "Assessment assigned".
- **A deadline carries its action.** "Complete your assessment by Sep 28", not "Assessment due Sep 28".
- **Step counts go in brackets after the action** (§2.4.2's copy fix), not after a `·`.
- **Never restate the status.** "Additional information required" and "Background check required" repeat `Action required` without saying what to do. If the specific item isn't known, the data isn't ready for this line.

ℹ️ Interview lines belong to the interview statuses, not standalone `Action required`: "Schedule your interview by Sep 30" goes with `Interview · Action required`, and a scheduled interview's date and time ("Sep 30 at 2 PM EDT") goes with `Interview scheduled`, since nothing is owed.

⚠️ **Scope:** two candidate lines may not be application requirements. "Complete required training" — training is a contract concern ([`engagements.md` §6](engagements.md#6-training)). "Review and sign your documents" — signing belongs to the offer and contracting step ([`engagements.md` §4](engagements.md#4-offers-and-contracting)). Neither is approved until confirmed — see §8.
...
**Clearing the requirement:** once the professional completes an outstanding requirement, `Action required` is removed from `application-status` (the status becomes the plain stage label: `Applied`, or `Interview scheduled` once the interview is booked), and `supporting-text` re-evaluates against §2.4.1's matrix for that now-plain status — typically becoming omitted (e.g. the plain `Applied` row) rather than continuing to reference the now-resolved requirement or its now-stale progress count. It must never keep showing the cleared requirement or a stale count.
...
> 🙋 The `Action required` indicator has no backing value in the [`engagements.md` §8](engagements.md#8-application-status-enum) enum yet — it remains a proposed display-only extension. See the related item in §8 of this doc.
>
> 🙋 Whether `supporting-text` should ever name the review owner after an interview completes (e.g. "Awaiting partner review" vs. "Awaiting Verita review"), or should always stay owner-agnostic ("Interview completed" alone) — the post-interview status itself is resolved (`In review`), but whether/when the owner-specific copy variants in §2.4.1's matrix are shown is still to be discussed with the team.
...
| ------------------- | ----------------- |
| `APPLIED`           | Applied           |
| `SCORING_PENDING`   | Applied           |
| `INTERVIEW_PENDING` | Interview scheduled (`Interview · Action required` while the professional still has to book it) |
...
✅ **Resolved (2026-09-23) — no `In review` before an interview:** everything between submission and a decision reads `Applied`, whichever of `SCORING_PENDING` or `UNDER_REVIEW` the system is in. `In review` is used only after a completed interview, when the professional has done their part and is waiting for an answer (§2.4.1).

Plain `Applied` (no outstanding requirement) has no `supporting-text`, per §2.4's value rule and §2.4.1's matrix: submission and review-start events ("Applied 4 days ago", "Review started 2 days ago") are bare timestamps with no guidance attached. Those dates remain available on the application detail page.

**`· Action required` is a display-only suffix, not a new enum value:** when an application is in the interview stage (`INTERVIEW_PENDING`) and the professional still has to book the interview, `application-status` renders the mapped label with an `· Action required` suffix: `Interview · Action required` (§2.4.1's status matrix). Before an interview, an application with an outstanding requirement reads just **`Action required`**, not `Applied · Action required`: with `Applied` as the only pre-decision status, the prefix adds nothing (✅ **Resolved (2026-09-23)**). This suffix is composed at display time from the mapped label above plus the existence of an outstanding requirement — it does not require (and must not be implemented as) a separate system status value. The underlying system status is unchanged throughout; only the card's rendering of it changes. This is a proposed extension to the current enum's display rules, not yet confirmed (§2.4.1's open questions).
...
| **Action required**             | `warning` | Needs the professional's attention — an outstanding requirement exists (§2.4.1).                                |
| **Interview scheduled**         | `success` | Healthy, forward-moving progress — a confirmed interview is active momentum.                                    |
| **In review**                   | `success` | Healthy, forward-moving progress — the interview is done and the professional is waiting for an answer. Mirrors [`contract-card.md` §3.1.2](contract-card.md#312-finalized-element-behaviors)'s `Active` → `success` precedent. |
...
| **Closed**                      | `neutral` | Terminal because the opportunity closed, not because of the professional or a decision about them — muted, consistent with `Not selected` and `Withdrawn`. |

This follows [`contract-card.md` §3.1.2](contract-card.md#312-finalized-element-behaviors)'s established rationale categories rather than picking tones ad hoc: `info` reserved for `Applied` — the one state with no forward-momentum signal yet, just submitted and waiting — `success` for every state that's healthy and actively progressing (`Interview scheduled`, `In review`, `Offer received`) — reusing the Contract card's `Active` → `success` precedent, where `success` means "going well right now," not exclusively "arrived at the final outcome" — `warning` for anything carrying `· Action required` (an outstanding requirement the professional owes), `purple` reserved for `On hold` as its own distinct parked/awaiting-decision state rather than reusing `warning`, and `neutral` for all three terminal states (`Not selected`, `Withdrawn`, and `Closed` are deliberately treated the same — a rejection is not a "failure" tone, a self-initiated withdrawal isn't either, and an opportunity closing says nothing about the professional, so none uses `destructive`). This card has no state that maps to `destructive`, unlike the Contract card's `Terminated` — there's no Applications status that represents an application ending in a way that should read as negative to the professional rather than simply concluded.

> ⚠️ **Decision needed:** [`engagements.md` §8](engagements.md#8-application-status-enum) leaves `ON_HOLD`'s placement in the suggested lifecycle unresolved, and the pre-offer suggested lifecycle in [`engagements.md` §3](engagements.md#3-applications) (`Interested → Applied → Interview → Selected → Offer → Contracting → Onboarding → Active → Completed`) predates and doesn't fully align with the §8 enum. This card follows the §8 enum/label table above as authoritative for status display; do not build stage-progress UI (e.g. a stepper) against the suggested lifecycle until it's reconciled against §8 — and, independent of that reconciliation, never build one that assumes `Interview` is a required stage (§3.2).
...
- Stop rendering that application as a row, in both the Engagements → Applications list and the Home preview. It isn't counted in the Applications total either.
- Never present the Applications card itself as also being the Offer card, and never add a "View offer" action to the row. Once `ACCEPTED`, the corresponding Offer surfaces separately per [`engagements.md` §4](engagements.md#4-offers-and-contracting) (Home module 6, per [`dashboard.md` §6](dashboard.md#6-information-architecture)), with its own expiration-focused content per that section's resolved notes, not a match tier or application-stage language.

✅ **Resolved (2026-09-23):** this follows [`engagements.md` §2](engagements.md#2-engagement-views)'s "progressive destinations" rule, which replaces the earlier "Applications preserves history" rule. An application stays in `Applications` while the application process is active or when it ends without an offer. Once an offer is issued, the opportunity moves to `Offers`, and later to `Contracts`. The `Offer received` label and its tone (§3) apply only where the application record still renders, such as application detail.
...
- **Interview required:** the application progresses through the `Interview` stage (`Interview · Action required` / `Interview scheduled`, then `In review`, per §2.4.1) before reaching a terminal or offer state.
- **Interview not required:** the application progresses directly from `Applied` to `Offer received` (or a terminal state) without ever passing through `Interview` — this is a valid, expected path, not a skipped or incomplete one.
- **Multiple interviews:** when an opportunity requires more than one interview, the application-level status stays in the interview stage (`Interview · Action required` or `Interview scheduled` for the next one) while any interview in the set is still pending or scheduled — individual interview instances are tracked as sub-detail within that stage, not as separate application-level statuses or additional rows on this card. Once the full set of required interviews is completed, the application-level status becomes `In review` until there's an outcome, the same as the single-interview case (§2.4.1).

**Card consequence:** the Applications card must not assume or render a fixed sequence of stages where `Interview` always appears between `Applied` and `Offer received`. Concretely:

- No universal application-progress stepper/timeline component should hardcode `Interview` as a required step — a stepper built against a fixed `Applied → Interview → Offer received` sequence would misrepresent every application whose opportunity doesn't require an interview.
...
Alongside the row click target, the card exposes a secondary `actions-menu` (`···`/overflow trigger) revealed on hover/focus with the row's hover feedback above — never permanently visible, so it doesn't compete with the row's own click target for attention on a dense list (§6.2).
...
> ✅ **Resolved (2026-09-24) — Home preview mirrors Engagements → Applications → `Open`:** the Home "Your applications" preview (§6 module 5 of [`dashboard.md`](dashboard.md#6-information-architecture), titled "Active Applications" in Figma) shows the first 3 applications of the `Open` filter, in that filter's default sort: action needed first ([`engagements.md` §3.1](engagements.md#31-application-filters)). Home and Engagements therefore never disagree about which applications are active or which comes first. `Not moving forward` applications don't appear on Home, and neither do applications that produced an offer (they've moved to `Offers`). Below the list, `View All` routes to Engagements → Applications with the `Open` filter selected; past applications are one click away under `Not moving forward`. This supersedes the earlier rule (most recently applied first, any status, including `Withdrawn` and `Not selected`), which put closed applications under an "Active" heading.
