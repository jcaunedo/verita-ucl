...
| View            | Meaning                                                                                                                                                                                                                                                                                                                                                                        |
| --------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Applications    | Specific opportunities the user has pursued. All opportunities the user has applied to, including active and historical applications and their current stage — user-facing labels: `Applied`, `In review`, `Interview`, `On hold`, `Not selected`, or `Withdrawn` (backed by the system status enum, §8). Rejected and withdrawn applications remain visible here rather than being removed from the user's history. |
...
>
> This is the one place an Engagement view and an Opportunity type ([dashboard.md §11.1](dashboard.md#111-opportunity-types)) share a name — `Talent Network` names both the work-arrangement type on the `Opportunity` and the Engagement view showing the user's memberships in that type. This is intentional, not the taxonomy collision §1 otherwise warns against: every other Opportunity type (`Project-based`, `One-time`, `Full Time`) still has no dedicated Engagement view of its own — a professional's Project-based work shows up under `Applications`/`Contracts` like any other opportunity type. `Talent Network` gets a dedicated view specifically because membership itself (not an application or contract event) is the object worth surfacing.
>
